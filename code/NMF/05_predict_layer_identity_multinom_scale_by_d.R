...
for (i in seq_along(sfe_list)){
    ...
    xen.factors.normed <- factors/nmf.mod$d
...
for (i in seq_along(sfe_list)){
    # sfe <- sfe_list[[i]]
    # layers <- 1:6
    # layers <- c(layers, "WM") # need to change this when working with bayesspace bc the layers are not named
    # 
    # layer_plts <- list()
