...
if (xen){ # xenium 
    ...
    sfe.paths <- system("ls processed-data/cindy/visium/", intern=TRUE)
    sfe.paths <- sfe.paths[grepl("*-with-banksy-domains.RDS", sfe.paths)]
    sfe.paths <- paste0("processed-data/cindy/visium", intern=TRUE)
    y_reverse <- TRUE
}
...
for (i in 1:length(sfes)){
    ...
    p1 <- make_escheR(sfe, y_reverse=y_reverse) %>%
        #add_ground(var="clust_M1_lam0.9_k50_res1.2", stroke=0.075)+
        add_fill(var=clustName, point_size=0.5)+
        scale_fill_discrete()
