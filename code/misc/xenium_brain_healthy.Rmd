...
# Moran test 
sfe.healthy <- runUnivariate(sfe.healthy, "moran.test", features = genes_use, colGraphName = "knn5",
                     BPPARAM = MulticoreParam(2))

rowData(sfe.healthy)$moran.test_Moran.I.statistic_sample01[!is.na(rowData(sfe.healthy)$moran.test_Moran.I.statistic_sample01)]
...
# Trying GLM-PCA
```{r}
sfe.healthy<- scry::nullResiduals(sfe.healthy, assay="counts", type="deviance")
sfe.healthy<- scater::runPCA(sfe.healthy, ncomponents = 50,
                      ntop = 1000,
                      exprs_values = "binomial_deviance_residuals",
                      scale = TRUE, name = "GLM-PCA",
                      BSPARAM = BiocSingular::RandomParam())
colData(sfe.healthy)$clust.glmpca <- clusterRows(reducedDim(sfe.healthy, "GLM-PCA")[,1:15],
                                    BLUSPARAM = SNNGraphParam(
                                        cluster.fun = "leiden",
                                        cluster.args = list(
                                            resolution_parameter = 0.5,
...
#plotPCA(sfe.healthy, ncomponents = 4, colour_by = "clust.glmpca", rasterise = FALSE)
#plotPCA(sfe.healthy, ncomponents = 5, colour_by = "subsets_negProbe_percent", rasterise = FALSE)
plotSpatialFeature(sfe.healthy, "clust.glmpca", colGeometryName = "cellSeg")

glmpca.markers <- scran::findMarkers(sfe.healthy, groups = colData(sfe.healthy)$clust.glmpca,
                       test.type = "wilcox", pval.type = "all", direction = "up")

genes_use_glmpca <- vapply(glmpca.markers, function(x) rownames(x)[1], FUN.VALUE = character(1))
plotExpression(sfe.healthy, genes_use_glmpca, x = "clust.glmpca", point_fun = function(...) list())
