...
computeGlobalEdgeConstraint <- function(tri, vertex){
    ...
    global_distance_constraint <- global_mean + 0.25*alpha * global_variation
...
```{r}
# let's see why the heck some of these hugely long edges are not getting pruned >:( 
...
which.max(globalPrunedtri$arcs$edge.lengths)
# [1] 125
globalPrunedtri$arcs[125,]
       # from  to edge.lengths
# 840-842  840 842     7634.015

nn <- rbind(edges[which(tri$arcs$from == 840),],
                edges[which(tri$arcs$to == 840),])

global_mean <- mean(tri$arcs$edge.lengths)
local_mean <- mean(nn$edge.lengths)
global_variation <- sd(tri$arcs$edge.lengths)

alpha <- global_mean / local_mean

cons <- global_mean + 0.25*alpha*global_variation

tri.max <- globalPrunedtri
tri.max$arcs <- tri.max$arcs[which.max(tri.max$arcs$edge.lengths),]

from.x <- tri.max$x[tri.max$arcs$from]
from.y <- tri.max$y[tri.max$arcs$from]

to.x <- tri.max$x[tri.max$arcs$to]
to.y <- tri.max$y[tri.max$arcs$to]

sqrt((from.x-to.x)^2 + (from.y-to.y)^2)
# [1] 39.08771
# but in the tri.max object, this edge length is like in the 7000s. 
# something has to be wrong with getEdgesSpatialDistance...

plotDelaunay(tri.max, br6471_p)
